...
import WebSocketPipelineTracker from "@/components/WebSocketPipelineTracker";
import { Asset, AttackChain, Finding } from "@/types";
...
const severityColor: Record<string, { badge: string; row: string }> = {
  ...
  medium: {
    ...
    row: "bg-yellow-500/[0.06]",
  },
  low: {
    badge: "bg-blue-500/20 text-blue-400 border-blue-500/30",
    row: "bg-blue-500/[0.06]",
  },
  info: {
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            <span
              className={`font-mono text-[10px] uppercase tracking-[0.2em] ${bs.text}`}
            >
              Chain {String(index + 1).padStart(2, "0")}
            </span>
            <Badge
              variant="outline"
              className={`font-mono text-[10px] px-1.5 py-0 ${bs.text} border-current`}
            >
              {bs.label}
            </Badge>
          </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right">
            ...
            <div
              className={`font-mono text-3xl font-bold tabular-nums leading-none mt-0.5 ${bs.text}`}
            >
              {Math.round(chain.blast_radius_score * 100)}%
            </div>
          </div>
          <ChevronRight
            className={`w-4 h-4 text-muted-foreground/50 transition-transform duration-200 ${expanded ? "rotate-90" : ""}`}
          />
        </div>
      </button>
                  ...
                  {chain.pivot_path.map((step, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-5 h-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold shrink-0 border ${bs.border} ${bs.text} bg-background`}
                        >
                          {step.step}
                        </div>
                        {i < chain.pivot_path.length - 1 && (
                          <div
                            className="w-px bg-border/40 my-1 flex-1"
                            style={{ minHeight: 12 }}
                          />
                        )}
                      </div>
                      <div
                        className={`${i < chain.pivot_path.length - 1 ? "pb-4" : "pb-0"} min-w-0`}
                      >
                        <div className="font-mono text-sm text-foreground font-medium">
                          {step.asset}
                        </div>
                        <div className={`font-mono text-xs mt-0.5 ${bs.text}`}>
                          {step.action}
                        </div>
              ...
              <div className="px-6 py-5 border-t border-border/40">
                ...
                <div className="space-y-2">
                  {chain.reachable_sensitive_stores.map((store, i) => (
                    <div
                      key={i}
                      className="flex gap-3 items-start p-3 rounded-lg bg-black/20 border border-border/30"
                    >
                      <Badge
                        variant="outline"
                ...
                <div className="px-6 py-3 flex flex-wrap gap-1.5 items-center border-b border-border/40">
                  ...
                  {chain.mitre_techniques.map((t) => (
                    <Badge
                      key={t}
                      variant="outline"
                      className="font-mono text-[10px] text-muted-foreground px-1.5 py-0"
                    >
                      {t}
                    </Badge>
                  ))}
...
export default function Home() {
  ...
  const [twPhase, setTwPhase] = useState<"typing" | "pausing" | "deleting">(
    "typing",
  );
  ...
  const fetchResults = useCallback(async (eid: string) => {
    ...
    try {
      ...
      const [fData, cData, aData] = await Promise.all([
        fRes.json(),
        cRes.json(),
        aRes.json(),
      ]);
  ...
  const handleRun = async () => {
    ...
    ws.onerror = () => {
      /* non-fatal, POST response is fallback */
    };
    ...
    <div className="min-h-screen bg-transparent flex flex-col">
      ...
      <main
        className={[
          ...
          stage === "loading" ? "justify-start pt-6" : "justify-center",
        ].join(" ")}
      >
        <AnimatePresence mode="wait">
          ...
          {stage === "search" && (
            ...
            >
              ...
              <p className="font-sans text-muted-foreground text-base font-normal mb-12">
                Enter the primary target URL to begin
              </p>
            ...
            >
              ...
              >
                ...
                <TagInput
                  ...
                  onRemove={(v) =>
                    setCloudAccounts((p) => p.filter((x) => x !== v))
                  }
                ...
                <div className="border-t border-border pt-8 space-y-8">
                  ...
                  <TagInput
                    ...
                    onRemove={(v) =>
                      setForbiddenHosts((p) => p.filter((x) => x !== v))
                    }
            ...
            >
              ...
              <div className="space-y-4 w-full text-left">
                  ...
                  >
                    {(() => {
                      const safeIndex = Math.min(
                        Math.max(loadingPhase, 0),
                        LOADING_PHASES.length - 1,
                      );
                      const Icon = LOADING_PHASES[safeIndex].icon;
                      return (
                        <>
                          <Icon className="w-4 h-4 text-primary" />
                          <span>{LOADING_PHASES[safeIndex].label}</span>
                          <motion.span
                ...
                <motion.div
                  ...
                  className="w-full"
                >
                  <WebSocketPipelineTracker ws={wsClient} />
                </motion.div>
              </div>
            </motion.div>
          )}
          ...
          {stage === "results" && (
            <motion.div
              key="results"
              className="w-full max-w-5xl mt-4 pb-16"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            >
              {/* URL bar */}
                  ...
                  <div className="flex gap-1 bg-muted/50 rounded-lg p-1">
                    {(["recon", "exploit"] as const).map((tab) => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-md font-mono text-xs uppercase tracking-wider transition-all ${
                          activeTab === tab
                        ...
                        <thead className="bg-muted sticky top-0">
                          <tr>
                            {[
                              ...
                              <th
                                key={h}
                                className="px-4 py-3 text-left text-xs font-mono uppercase tracking-wider text-muted-foreground font-semibold"
                              >
                                {h}
                              </th>
                            ))}
                        ...
                        <tbody>
                          {assets.map((asset) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3">
                                <span
                                  className={`font-mono text-sm font-bold tabular-nums ${surfaceScoreStyle(asset.attack_surface_score)}`}
                                >
                        ...
                        <tbody>
                          {vulnerabilities.map((vuln) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 font-mono text-xs text-primary max-w-45 truncate">
                                {vuln.affected_url}
                              </td>
                              <td className="px-4 py-3">
                                {vuln.exploitable ? (
